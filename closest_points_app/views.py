...
class PointSetAPIView(APIView):
    def post(self, request):
        serializer = PointSetSerializer(data=request.data)
        if serializer.is_valid():
            ...
            point_set = PointSet(points=serializer.validated_data['points'], closest_points=closest_points)
            point_set.save()
    ...
    def find_closest_points(coordinates):
        ...
        closest_points = [','.join(str(coord) for coord in coordinates[idx]) for idx in closest_indices]
